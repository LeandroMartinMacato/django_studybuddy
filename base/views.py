...
def home(request):
    ...
    topic = Topic.objects.all()[0:5] # get all topic from db
...
def room(request , pk):
    ...
    # room_messages = room.message_set.all().order_by('-created') # give all message to the room above
    room_messages = room.message_set.all() # give all message to the room above
...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        topic , created = Topic.objects.get_or_create(name=topic_name) # get the topic if it is new the 'created' var will be true
        Room.objects.create(
            host = request.user , 
            topic = topic,
...
def userProfile(request , pk):
    ...
    rooms = user.room_set.all() # get all children of an object
...
def deleteMessage(request ,pk):
    ...
    if request.user != message.user: # if user is trying to edit other people's room
        return HttpResponse("You are not allowed here!")
